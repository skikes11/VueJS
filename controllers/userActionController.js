...
const userActionController ={
    ...
    PaymentOrder: async (req, res, User_ID) => {
        try {
            ...
            const orderItems = await OrderItems.findOne({ Order_ID : order._id });  // get order items to check null and minus quantity product after user payment
            ...
            if(orderItems){
                ...
                for(const item in orderItems){
                    ...
                    product.total_quantity = product.total_quantity - item.quantity;
                    product.save();
                }
